const version = '4.1.0';
var API_BASE_URL = 'http://localhost:9001'
...
module.exports = {
  ...
  checkToken: (token) => {
    return request('/admin/check-token', true, 'get', {
      token
    })
  },
  ...
  login_tt: (code) => {
    return request('/admin/microapp/login', true, 'post', {
      code
    })
  },
  login_username: (data) => {
  ...
  register_tt: (data) => {
    return request('/admin/microapp/register', true, 'post', data)
  },
  ...
  userDetail: (token) => {
    return request('/admin/detail', true, 'get', {
      token
    })
  },
  userWxinfo: (token) => {
